...
//
//   Beat 0 (S_LOAD): eliminate x0 by Gaussian elimination -> reduced 2x2
//                    system in (x1,x2); form determinant + Cramer numerators;
//                    latch the raw (unshifted) results.
//   Beat 1 (S_OUT) : exact-divide (modular inverse, no divider) -> x1,x2;
//                    back-substitute x0; classify; drive outputs.  The divide
//                    lives here because this beat has timing slack.
//=========================================================================
...
//-------------------------------------------------------------------------
// Beat-0 -> beat-1 pipeline registers (raw values, shift happens in beat 1)
//-------------------------------------------------------------------------
...
// by an even denominator we first strip its power-of-2 factor (shift both
...
// Explicit slice-mux: smaller on this library than an inferred barrel shifter.
...
// x0_num holds a0*x0, |a0*x0| <= 124 -> fits 8 bits.
...
assign m11 = a0*b1 - a1*b0;   // x1 coefficient, row 1
assign m12 = a0*c1 - a1*c0;   // x2 coefficient, row 1
assign v1  = a0*d1 - a1*d0;   // right-hand side, row 1
assign m21 = a0*b2 - a2*b0;   // x1 coefficient, row 2
assign m22 = a0*c2 - a2*c0;   // x2 coefficient, row 2
assign v2  = a0*d2 - a2*d0;   // right-hand side, row 2

logic signed [11:0] det;
logic signed [15:0] num_x1, num_x2;

assign det    = m11*m22 - m21*m12;   // 2x2 determinant
assign num_x1 = v1*m22  - v2*m12;    // Cramer numerator for x1
assign num_x2 = v2*m11  - v1*m21;    // Cramer numerator for x2
...
//          (from the latched raw beat-0 registers; this beat has slack so
//           the trailing-zero count + shifts are cheap to place here)
//=========================================================================
...
// x1,x2 = num/det  via the shared modular inverse of det
logic [3:0] det_tz;
logic [5:0] det_inv;
logic signed [5:0] sol1, sol2;

assign det_tz  = count_tz12(det_r);
assign det_inv = inv_mod64(arshift6_w12(det_r, det_tz));
assign sol1    = mul_mod64(arshift6_w16(num_x1_r, det_tz), det_inv); // x1
assign sol2    = mul_mod64(arshift6_w16(num_x2_r, det_tz), det_inv); // x2

// x0 by back-substitution into eq0:  a0*x0 = d0 - b0*x1 - c0*x2
// x0_num holds a0*x0; intermediate terms wrap mod 256 (2's-complement exact).
...
//-------------------------------------------------------------------------
// Outputs: zero unless out_valid is high (protocol).  out_data is NOT gated
// by exc_w: the checker only compares out_data for the unique case, so
// leaving sol* driven for infinite/none is harmless and saves mux gates.
//-------------------------------------------------------------------------
