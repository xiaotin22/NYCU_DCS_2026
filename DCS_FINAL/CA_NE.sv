module CA #(
    ...
    logic [ACC_W-1:0]    mult_max_mask;
    mult_tag_t           mult_out_tag;
    logic [1:0]          mult_out_tag_raw;
    ...
    always_comb begin
        ...
        case (state_q)
            ...
            S_ATT_READ: begin
                if (att_issue_valid) begin
                    mult_issue_valid = 1'b1;
                    mult_issue_op    = 2'b00;
                    mult_issue_A     = rd_data;
                    mult_issue_tag   = MT_ATT_Q;
                    mult_issue_first = att_issue_first;
                end
            end
    ...
    att_full_parallel #(
        ...
        .wk_data     (param2_q),
        .wv_data     (param3_q),
        .q_sum_data  (mult_lane_data),
        .q_max_mask_data(mult_max_mask),
        .out_valid   (att_valid),
...
module att_full_parallel #(
    ...
    input  logic [(MAT_SIZE*ACC_W)-1:0] q_sum_data,
    input  logic [ACC_W-1:0]     q_max_mask_data,
    output logic                 out_valid,
    ...
    logic [1:0]           score_mha_pipe_q;
    logic [4:0]           score_first_pipe_q;
    ...
    logic                 score_valid_q;
    logic [2:0]           score_v_booth0_q [0:MAT_SIZE-1];
    logic [2:0]           score_v_booth1_q [0:MAT_SIZE-1];
    ...
    function automatic logic [PROJ_SHIFT_W-1:0] proj_pot_shift(input proj_mag_t max_abs);
        ...
        begin
            ...
            for (int b = 0; b < PROJ_W; b++) begin
                if (max_abs[b]) begin
                    msb = b[PROJ_SHIFT_W-1:0];
                end
            end
            proj_pot_shift = (msb > PROJ_SHIFT_TWO) ? (msb - PROJ_SHIFT_TWO) : '0;
        end
    endfunction

    function automatic logic [255:0] quant_q_sum(
        input acc_vec_t src,
        input logic [PROJ_SHIFT_W-1:0] shift
    );
        proj_t value;
        begin
            quant_q_sum = 256'd0;
    ...
    function automatic score_pair_t score_pair_sum(input prod_t a, input prod_t b);
        score_pair_sum = score_pair_t'(a) + score_pair_t'(b);
    endfunction
    ...
    function automatic logic [2:0] booth0_from_vec(
        input logic [255:0] vec,
        input integer       idx
    );
        s4_t v_value;
        begin
            v_value = get_s4(vec, idx);
            booth0_from_vec = {v_value[1], v_value[0], 1'b0};
        end
    endfunction

    function automatic logic [2:0] booth1_from_vec(
        input logic [255:0] vec,
        input integer       idx
    );
        s4_t v_value;
        begin
            v_value = get_s4(vec, idx);
            booth1_from_vec = {v_value[3], v_value[2], v_value[1]};
        end
    endfunction

    function automatic score_prod_t calc_final_prod(
        input score_t       score_mat [0:MAT_SIZE-1],
        input logic [2:0]   v_booth0 [0:MAT_SIZE-1],
        input logic [2:0]   v_booth1 [0:MAT_SIZE-1],
        input integer       row,
        input integer       lane,
        ...
        begin
            pp0 = booth_pp(score_mat[(row * ROW_ELEM) + tap_idx],
                           v_booth0[(tap_idx * ROW_ELEM) + lane]);
            pp1 = booth_pp(score_mat[(row * ROW_ELEM) + tap_idx],
                           v_booth1[(tap_idx * ROW_ELEM) + lane]);
        ...
        for (int row = 0; row < ROW_ELEM; row++) begin
            for (int lane = 0; lane < ROW_ELEM; lane++) begin
                ...
                score_lo_next[(row * ROW_ELEM) + lane] =
                    score_half_t'(score_pair_sum(score_prod_q[(row * ROW_ELEM) + lane][0],
                                                  score_prod_q[(row * ROW_ELEM) + lane][1])) +
                    score_half_t'(score_pair_sum(score_prod_q[(row * ROW_ELEM) + lane][2],
                                                  score_prod_q[(row * ROW_ELEM) + lane][3]));
                score_hi_next[(row * ROW_ELEM) + lane] =
                    score_half_t'(score_pair_sum(score_prod_q[(row * ROW_ELEM) + lane][4],
                                                  score_prod_q[(row * ROW_ELEM) + lane][5])) +
                    score_half_t'(score_pair_sum(score_prod_q[(row * ROW_ELEM) + lane][6],
                                                  score_prod_q[(row * ROW_ELEM) + lane][7]));
                score0_next[(row * ROW_ELEM) + lane] = score_mha_pipe_q[1] ?
                    att_score_value(score_sum_t'(score_lo_q[(row * ROW_ELEM) + lane])) :
                    att_score_value(score_sum_t'(score_lo_q[(row * ROW_ELEM) + lane] +
                                                 score_hi_q[(row * ROW_ELEM) + lane]));
                score1_next[(row * ROW_ELEM) + lane] = score_mha_pipe_q[1] ?
                ...
                for (int tap = 0; tap < ROW_ELEM; tap++) begin
                    if (lane < 4) begin
                        final_prod_next[(row * ROW_ELEM) + lane][tap] =
                            calc_final_prod(score0_q, score_v_booth0_q, score_v_booth1_q, row, lane, tap);
                    end
                    else begin
                        final_prod_next[(row * ROW_ELEM) + lane][tap] =
                            calc_final_prod(score1_q, score_v_booth0_q, score_v_booth1_q, row, lane, tap);
                    end
                end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            score_part_valid_q <= score_prod_valid_q;
            ...
            score_mha_pipe_q   <= {score_mha_pipe_q[0], proj_mha};
            score_first_pipe_q <= {score_first_pipe_q[3:0], proj_first};
            ...
            for (int i = 0; i < MAT_SIZE; i++) begin
                score_v_booth0_q[i] <= booth0_from_vec(score_part_v_q, i);
                score_v_booth1_q[i] <= booth1_from_vec(score_part_v_q, i);
            ...
            out_first <= score_first_pipe_q[4];
            out_data <= final_next;
        end
    end
...
module att_kv_proj_pipe #(
    ...
    logic                 part_valid_q;
    logic                 sum_valid_q;
    logic [3:0]           mha_pipe_q;
    logic [3:0]           first_pipe_q;

    // K/V uses the same select -> product -> partial-sum -> sum rhythm as
    // mat_mac_pipe; the source operand is shared by all lanes in a row.
    ...
    function automatic proj_mag_t max_abs_next_pipe(input integer pipe);
        begin
            max_abs_next_pipe = '0;
            for (int i = 0; i < MAT_SIZE; i++) begin
                max_abs_next_pipe |= abs_proj(sum_next[pipe][i]);
            end
        end
    endfunction
    ...
    function automatic logic [255:0] quant_sum_pipe(
        input integer             pipe,
    ...
    function automatic s4_t weight_s4(input integer pipe, input integer lane, input integer tap);
        begin
            weight_s4 = (pipe == K_PIPE) ? get_s4(wk_data, (tap * ROW_ELEM) + lane) :
                                           get_s4(wv_data, (tap * ROW_ELEM) + lane);
        end
    endfunction
                ...
                for (int lane = 0; lane < ROW_ELEM; lane++) begin
                    for (int tap = 0; tap < ROW_ELEM; tap++) begin
                        prod_next[pipe][(row * ROW_ELEM) + lane][tap] =
                            prod_t'($signed(src_row_q[row][tap]) *
                                    $signed(weight_s4(pipe, lane, tap)));
                    end
                end
            end
        end
            ...
            for (int row = 0; row < ROW_ELEM; row++) begin
                for (int lane = 0; lane < ROW_ELEM; lane++) begin
                    ...
                    sum_next[pipe][(row * ROW_ELEM) + lane] =
                        proj_t'(part_q[pipe][(row * ROW_ELEM) + lane][0]) +
                        proj_t'(part_q[pipe][(row * ROW_ELEM) + lane][1]);
                end
            end
        end
    end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            sel_valid_q  <= in_valid;
            prod_valid_q <= sel_valid_q;
            part_valid_q <= prod_valid_q;
            sum_valid_q  <= part_valid_q;
            out_valid    <= sum_valid_q;

            mha_pipe_q   <= {mha_pipe_q[2:0], is_mha};
            first_pipe_q <= {first_pipe_q[2:0], in_first};
            ...
            for (int pipe = 0; pipe < PIPE_COUNT; pipe++) begin
                ...
                max_mask_q[pipe] <= max_abs_next_pipe(pipe);
            end

            out_mha   <= mha_pipe_q[3];
            out_first <= first_pipe_q[3];
            k_data <= quant_sum_pipe(K_PIPE, pot_shift(max_mask_q[K_PIPE]));
            v_data <= quant_sum_pipe(V_PIPE, pot_shift(max_mask_q[V_PIPE]));
        end
    end
...
module mat_mac_pipe #(
    ...
    output logic [ACC_W-1:0]     out_max_mask,
    ...
    logic [2:0][1:0] tag_pipe_q;
    logic [2:0] first_pipe_q;
    ...
    function automatic mag_t max_abs_sum(input acc_t lanes [0:MAT_SIZE-1]);
        begin
            max_abs_sum = '0;
            for (int i = 0; i < MAT_SIZE; i++) begin
                max_abs_sum |= abs_acc(lanes[i]);
            end
        end
    endfunction
    ...
    always_comb begin
        for (int lane = 0; lane < ROW_ELEM; lane++) begin
            for (int tap = 0; tap < DOT_SIZE; tap++) begin
                b_sel_next[lane][tap] = sel_b4(in_data_B, op, lane, tap);
            end
        end
            ...
            for (int lane = 0; lane < ROW_ELEM; lane++) begin
                for (int tap = 0; tap < DOT_SIZE; tap++) begin
                    a_sel_next[(row * ROW_ELEM) + lane][tap] =
                        sel_a4(in_data_A, op, (row * ROW_ELEM) + lane, tap);
    ...
    assign out_lane_data = pack_sum(sum_q);
    assign out_max_mask = max_mask_q;
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            prod_valid_q <= sel_valid_q;
            part_valid_q <= prod_valid_q;
            tag_pipe_q   <= {tag_pipe_q[1:0], in_tag};
            first_pipe_q <= {first_pipe_q[1:0], in_first};

            out_valid <= part_valid_q;

            out_tag   <= tag_pipe_q[2];
            out_first <= first_pipe_q[2];
    ...
    always_ff @(posedge clk) begin
        ...
        max_mask_q <= max_abs_sum(sum_next);
    end
...
module act_4stage_parallel #(
    ...
    localparam int ACC_VEC_W  = MAT_SIZE * ACC_W;
    localparam int PSUM_W     = ACC_W + 1;
    localparam int AVG_SUM_W  = ACC_W + 3;
    ...
    function automatic avg_t calc_final_avg(
        ...
        begin
            ...
            calc_final_avg = (&act_sel) ? avg_t'(sum >>> 4) :
                                          avg_t'(sum >>> 3);
        end
    endfunction
...
module pot_5stage_parallel #(
    ...
    logic [255:0]  quant_q;
    logic [255:0]  quant_pipe_q;
    logic [255:0]  quant_pipe2_q;
    logic [3:0][1:0] tag_pipe_q;
    ...
    assign max2_next = mag_or4(
        mag_or4(max0_q[0],  max0_q[1],  max0_q[2],  max0_q[3]),
        mag_or4(max0_q[4],  max0_q[5],  max0_q[6],  max0_q[7]),
        mag_or4(max0_q[8],  max0_q[9],  max0_q[10], max0_q[11]),
        mag_or4(max0_q[12], max0_q[13], max0_q[14], max0_q[15])
    );
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            tag_pipe_q <= {tag_pipe_q[2:0], in_tag};

            // Stage 0/1: keep the source data once and build the max mask from input.
            max0_data_q <= in_data;

            for (int g = 0; g < MAX_L0; g++) begin
                max0_q[g] <= mag_or4(abs_acc(get_acc(in_data, (g * 4) + 0)),
                                      abs_acc(get_acc(in_data, (g * 4) + 1)),
                                      abs_acc(get_acc(in_data, (g * 4) + 2)),
                                      abs_acc(get_acc(in_data, (g * 4) + 3)));
            end

            // Stage 2: 16 OR groups -> one global magnitude-bit mask, then quantize.
            quant_q <= quant_all(max0_data_q, pot_shift(max2_next));

            // Stages 3/4: keep POT latency aligned with the top-level RAM flow.
            quant_pipe_q <= quant_q;
            quant_pipe2_q <= quant_pipe_q;
            out_tag <= tag_pipe_q[3];
            out_data <= quant_pipe2_q;
        end
    end
endmodule
