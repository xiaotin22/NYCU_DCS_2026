...
- (d) 加 in_mode 做 circular shift，Tom vs Jerry 哪個面積小？**Ans：Tom 較小**。Jerry 在 4 種 mode 各開一套加法硬體（4 套）；Tom 先用 MUX 選資料、只保留一套加法器 → 面積較小。（參考面積 Tom≈25181、Jerry≈32941）
...
**Q8 Waveform（14%）** — FSM：IDLE/DIN/COM1/COM2/OUT，out_num 在 COM1 做 +2、COM2 做 >>1，in_reg 有號。**Ans 波形（10 cycle）**：
